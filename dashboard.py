...
if "session_initialized" not in st.session_state:
    ...
    # Clear portfolio file when page is freshly loaded
    if os.path.exists(PORTFOLIO_FILE):
        try:
            os.remove(PORTFOLIO_FILE)
            st.sidebar.success("✨ Fresh session started - Previous data cleared")
        except:
            pass

def fetch_crypto_data(crypto_name):
    """Fetch crypto data with cache busting for auto-refresh"""
    try:
        # Cache key with time-based refresh
        cache_key = f"{crypto_name}_{int(datetime.now().timestamp() / 300)}"
...
def get_market_cap_data():
    """Fetch global market cap data with auto-refresh"""
    try:
        cache_key = f"market_cap_{int(datetime.now().timestamp() / 300)}"
...
def get_top_gainers_losers():
    """Fetch top gainers and losers with auto-refresh"""
    try:
        cache_key = f"gainers_losers_{int(datetime.now().timestamp() / 300)}"
...
with st.sidebar:
    st.markdown("""
    <div style='text-align: center; padding: 30px 10px; margin-bottom: 30px; animation: fadeInDown 0.8s ease-out;'>
        <div style='font-size: 48px; color: #ccd12c; font-weight: 900; letter-spacing: 2px; text-transform: uppercase; animation: float 3s ease-in-out infinite; text-shadow: 0 0 20px rgba(208, 214, 30, 0.6), 0 0 40px rgba(0, 212, 255, 0.3), 0 0 60px rgba(0, 100, 200, 0.2);'>
            CRYPTALYZE
        </div>
        <div style='margin-top: 12px; letter-spacing: 2px; color: #ccd12c; font-weight: 600; text-shadow: 0 0 15px rgba(0, 212, 255, 0.5);'>
            ▬▬▬▬▬▬▬▬
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.session_state.auto_refresh_enabled = st.checkbox("Enable Auto-Refresh", value=st.session_state.auto_refresh_enabled)
    if st.session_state.auto_refresh_enabled:
        st.session_state.refresh_interval = st.slider("Refresh Interval (minutes)", min_value=5, max_value=60, value=st.session_state.refresh_interval, step=5)
        st.info(f"📊 Data will refresh every **{st.session_state.refresh_interval} minutes**")
    
    # Display last refresh time
    time_since_refresh = (datetime.now() - st.session_state.last_refresh).total_seconds() / 60
    st.caption(f"Last refreshed: {time_since_refresh:.1f} min ago")
...
if st.session_state.auto_refresh_enabled:
    ...
    if time_since_refresh >= st.session_state.refresh_interval:
        ...
        st.sidebar.success(f"⏱️ Next refresh in {remaining:.1f} min")

# Welcome header on main page
st.markdown("""
<div style='padding: 30px 0; margin-bottom: 38px; animation: fadeInDown 0.8s ease-out;'>
    <div style='font-size: 32px; color: #ffffff; font-weight: 800; margin-bottom: 8px;'>
        ✨ ENTER THE TRADING ARENA 
    </div>
    <div style='font-size: 22px; color: #49678c; font-weight: 300;'>
...
with tab1:
    ...
    selected = st.multiselect("View Markets", list(crypto_map.keys()), default=["Bitcoin", "Ethereum"], label_visibility="collapsed")
    ...
    adv_selected = st.multiselect("Advanced Charts (MACD, Bollinger Bands)", list(crypto_map.keys()), default=[], label_visibility="collapsed", key="advanced_charts")
...
with tab2:
    ...
    if not st.session_state.portfolio:
        st.markdown("""
        <div style='background: linear-gradient(135deg, rgba(0, 212, 255, 0.1) 0%, rgba(0, 150, 200, 0.05) 100%); border: 1.5px solid rgba(0, 212, 255, 0.3); border-radius: 12px; padding: 40px; text-align: center; margin: 20px 0;'>
            <div style='font-size: 48px; margin-bottom: 20px;'>📊</div>
            <div style='font-size: 20px; font-weight: 800; color: #00d4ff; margin-bottom: 10px;'>No Holdings Yet</div>
            <div style='font-size: 14px; color: #c5d3e0; line-height: 1.6;'>
                Get started by adding your first cryptocurrency holdings to your portfolio using the sidebar. Track your investments and get real-time market insights.
            </div>
        </div>
        """, unsafe_allow_html=True)
    else:
        ...
        with col3:
            la_time = datetime.now(pytz.timezone('America/Los_Angeles')).strftime("%H:%M:%S")
            st.metric("Last Update", la_time)
...
with tab3:
    ...
    selected = st.multiselect("Analyze Cryptos", list(crypto_map.keys()), default=["Bitcoin"], label_visibility="collapsed", key="signals")
    if not selected:
...
with tab4:
    ...
    selected = st.multiselect("View Analysis", list(crypto_map.keys()), default=["Bitcoin", "Ethereum"], label_visibility="collapsed", key="analysis")
    if not selected:
        st.info("Select at least one crypto to analyze")
    else:
        for crypto in selected:
...
with tab5:
    ...
    news_crypto = st.selectbox("Select Crypto for News", list(crypto_map.keys()), label_visibility="collapsed", key="news_crypto")
